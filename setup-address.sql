-- ============================================================================
--  RajkotMarket — Structured delivery address
--  Stores the Indian address fields (flat/house, building, street, area,
--  landmark, city, PIN) as JSONB next to the composed delivery_address
--  string, so the delivery partner panel can show labelled fields.
--  Old orders keep working: panels fall back to the plain string.
--  Requires: setup-offers.sql (this replaces checkout v3 with v4).
--  Safe to re-run: yes
-- ============================================================================

-- 1. ORDERS — structured address column
ALTER TABLE orders
  ADD COLUMN IF NOT EXISTS address_json JSONB;

-- 2. CHECKOUT v4 — also stores p_customer->'address_parts'
...
SELECT
  (SELECT count(*) FROM information_schema.columns
    WHERE table_name = 'orders' AND column_name = 'address_json') AS address_column;
