...
const { uploadJsonFile, deleteFile } = require('./drive');

const ARCHIVE_AFTER_DAYS = 15;
const MAX_PER_RUN = 500;   // safety cap: archive at most this many orders per run

// Retention purge for call recordings: delete the Drive audio + the index row
// once expires_at (created_at + 1 year) has passed. Best-effort per file so one
// failure doesn't block the rest. Returns how many rows were removed.
async function purgeExpiredRecordings() {
  const nowIso = new Date().toISOString();
  const { data: expired } = await supabase
    .from('call_recordings').select('id, drive_file_id')
    .lt('expires_at', nowIso).limit(500);
  if (!expired || !expired.length) return 0;
  let removed = 0;
  for (const r of expired) {
    try { if (r.drive_file_id) await deleteFile(r.drive_file_id); } catch (e) { continue; }
...
module.exports = async (req, res) => {
  ...
  try {
    // Always run the recording retention purge (independent of order archiving).
    const recordingsPurged = await purgeExpiredRecordings();
    ...
    if (!candidateIds.length) return res.json({ purged: 0, recordings_purged: recordingsPurged, message: 'Nothing to archive.' });
    ...
    if (!orders || !orders.length) return res.json({ purged: 0, recordings_purged: recordingsPurged, message: 'Nothing to archive.' });
