...
module.exports = async (req, res) => {
  ...
  // Env presence diagnostic — reports only whether each var is set and its
  // length, never the secret values. Remove once verified.
  if (req.query && req.query.selftest === 'env') {
    const look = k => ({ present: !!process.env[k], length: (process.env[k] || '').length });
    return res.json({
      selftest: 'env',
      GOOGLE_CLIENT_ID: look('GOOGLE_CLIENT_ID'),
      GOOGLE_CLIENT_SECRET: look('GOOGLE_CLIENT_SECRET'),
      GOOGLE_REFRESH_TOKEN: look('GOOGLE_REFRESH_TOKEN'),
      CRON_SECRET: look('CRON_SECRET'),
      GDRIVE_ARCHIVE_FOLDER_ID: look('GDRIVE_ARCHIVE_FOLDER_ID')
    });
  }
