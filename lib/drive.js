...
// body. Used to archive return QC photos before they're purged from Cloudinary.
// Returns { id, name, webViewLink }.
async function uploadBinaryFile(filename, mimeType, buffer) {
  const token = await getAccessToken();
  const folderId = process.env.GDRIVE_ARCHIVE_FOLDER_ID;
...
module.exports = { uploadJsonFile, uploadBinaryFile };
