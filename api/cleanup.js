// Nightly storage-cycle job (Vercel Cron → this endpoint).
//
...
//
// Security: this is a machine endpoint, authenticated by CRON_SECRET (not the
// browser guard / sessions). Vercel automatically sends
//   Authorization: Bearer <CRON_SECRET>
// on scheduled invocations when CRON_SECRET is set in the project env.

const supabase = require('../lib/supabase');
const { uploadJsonFile } = require('../lib/drive');

const ARCHIVE_AFTER_DAYS = 15;
const MAX_PER_RUN = 500;   // safety cap: archive at most this many orders per run

module.exports = async (req, res) => {
  // ---- auth: only Vercel Cron (or someone holding CRON_SECRET) may run this
