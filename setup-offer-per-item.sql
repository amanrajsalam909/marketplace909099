-- ============================================================================
--  RajkotMarket — Per-ITEM vendor offers (checkout v5)
--
--  Offers can now target a single product (offers.product_id). At checkout the
--  discount base for a product-scoped offer is that product line total in the
--  order; shop-wide offers (product_id NULL) still discount the whole subtotal.
--  Best single offer per shop still wins (one discount per order, unchanged
--  order schema). Requires setup-offers.sql + setup-address.sql first.
--  Safe to re-run.
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_offers_product ON offers (product_id) WHERE product_id IS NOT NULL;
...
BEGIN
  ...
  LOOP
    ...
    -- Offers may target a specific product (offers.product_id): the discount
    -- base is that product line total in this order. Shop-wide offers
    -- (product_id IS NULL) use the whole subtotal, exactly as before.
    ...
    LOOP
      IF v_offer.product_id IS NOT NULL THEN
        SELECT coalesce(round(sum(line_total), 2), 0) INTO v_base
          FROM tmp_valid WHERE vendor_id = v_vendor.id AND product_id = v_offer.product_id;
        IF v_base <= 0 THEN CONTINUE; END IF;   -- targeted product not in this order
      ELSE
        v_base := v_subtotal;
      END IF;
      v_d := CASE WHEN v_offer.discount_type = 'percent'
                  THEN round(v_base * v_offer.discount_value / 100, 2)
      ...
      END IF;
      IF v_d > v_base THEN v_d := v_base; END IF;   -- never exceed the offer base
...
-- VERIFICATION — expect product_id_column = 1
SELECT count(*) AS product_id_column
